...
class BusinessRulesTestCase(unittest.TestCase):
    ...
    @patch('src.services.reservas_services.guardar_reserva')
    @patch('src.services.reservas_services.obtener_reservas_relacionadas', return_value=[])
    @patch('src.services.reservas_services.obtener_cancha_por_id')
    @patch('src.services.reservas_services.obtener_socio_por_id')
    def test_reservation_checks_entities_overlap_and_historical_price(
        self, socio_get, cancha_get, related_get, save
    ):
        socio_get.return_value = {'id': 1, 'activo': True}
        cancha_get.return_value = {'id': 2, 'activa': True, 'precio_hora': 1500}
        save.side_effect = lambda data: {**data, 'id': 8}
    ...
    @patch('src.services.canchas_services.eliminar_cancha')
    @patch('src.services.canchas_services.contar_reservas_de_cancha', return_value=2)
    @patch('src.services.canchas_services.obtener_cancha_por_id', return_value={'id': 1})
    def test_court_delete_rejects_associated_reservations(self, get, count, delete):
